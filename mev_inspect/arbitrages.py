...
MAX_TOKEN_AMOUNT_DIFFERENCE = 0.01


def get_transaction_hash(swap: Swap):
    return swap.transaction_hash
...
def _get_arbitrages_from_swaps(swaps: List[Swap]) -> List[Arbitrage]:
    ...
    There are two types of swaps that are most common (99%+).
    Case I (fully routed):
    BOT -> A/B -> B/C -> C/A -> BOT
    ...
    There is only one correct way to route Case I, but for Case II the following valid routes could be found:
    ...
    for (start, ends) in start_ends:
        ...
        if route is not None:
            ...
            input_tokens_address, input_tokens_amount, \
                output_tokens_address, output_tokens_amount = _get_all_used_tokens(route)
            contracts_address = _get_all_contracts(route)

            arb = Arbitrage(
                swaps=route,
                block_number=route[0].block_number,
                timestamp=route[0].timestamp,
                transaction_hash=route[0].transaction_hash,
                account_address=route[0].from_address,
                status=status,
                contracts_address=contracts_address,
                input_tokens_address=input_tokens_address,
                input_tokens_amount=input_tokens_amount,
                output_tokens_address=output_tokens_address,
                output_tokens_amount=output_tokens_amount,
                gas_price=route[0].gas_price,
                gas_usage=route[0].gas_usage,
                block_position=route[0].transaction_position
            )
...
def _get_shortest_route(
    ...
    for end_swap in end_swaps:
        if _swap_out_match_swap_in(start_swap, end_swap):
    ...
    for next_swap in other_swaps:
        if _swap_out_match_swap_in(start_swap, next_swap):
...
def _get_all_start_end_swaps(swaps: List[Swap]) -> List[Tuple[Swap, List[Swap]]]:
    ...
    """
    pool_addresses = set(swap.contract_address for swap in swaps)
    valid_start_ends: List[Tuple[Swap, List[Swap]]] = []

    for index, potential_start_swap in enumerate(swaps):
        if potential_start_swap.from_address in pool_addresses:
            continue
        ...
        for potential_end_swap in remaining_swaps:
            if (
                    potential_start_swap.token_in_address == potential_end_swap.token_out_address
...
def _swap_out_match_swap_in(swap_out: Swap, swap_in: Swap) -> bool:
    return (
            ...
            and equal_within_percent(swap_out.token_out_amount, swap_in.token_in_amount,
                                     MAX_TOKEN_AMOUNT_DIFFERENCE)
    )


def _get_all_used_tokens(route: List[Swap]) -> (List[str], List[str], List[str], List[str]):
    used_input_tokens = []
    used_input_tokens_amount = []
    used_output_tokens = []
    used_output_tokens_amount = []
    for swap in route:
        used_input_tokens.append(swap.token_in_address)
        used_input_tokens_amount.append(swap.token_in_amount)
        used_output_tokens.append(swap.token_out_address)
        used_output_tokens_amount.append(swap.token_out_amount)

    return used_input_tokens, used_input_tokens_amount, used_output_tokens, used_output_tokens_amount


def _get_all_contracts(route: List[Swap]) -> List[str]:
    return list(set(swap.contract_address for swap in route))
